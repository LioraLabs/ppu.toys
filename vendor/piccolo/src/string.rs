            ...
            fn drop(&mut self) {
                match self.header.buffer {
                    Buffer::Indirect(ptr) => unsafe {
                        // LOCAL DELTA (PPU-97): explicit reborrow. Identical codegen;
                        // the implicit autoref is deny-by-default for a path dependency,
                        // where rustc does not cap lints the way it does for registry deps.
