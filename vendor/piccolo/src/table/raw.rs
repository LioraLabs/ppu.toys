...
impl<'gc> RawTable<'gc> {
    ...
    // LOCAL DELTA (PPU-97): this `set` and the `grow_array` helper below are
    // backported verbatim from piccolo master commit f617091a ("A lot of
    // improvements to RawTable", 2024-07-02) — 16 days after 0.3.3 shipped, and
    // never released. The 0.3.3 body's grow path had two arms, each doing exactly
    // one of the two things the following map insert requires: the array-growth
    // arm purged dead keys but reserved no capacity (panic "map slot must be
    // pre-reserved"), and the map-growth arm reserved capacity but purged no dead
    // keys (panic "all keys must be live when table is grown"). Master routes both
    // through `reserve_map`, which does both. See vendor/piccolo/README.md.
    pub fn set(
        &mut self,
        key: Value<'gc>,
        value: Value<'gc>,
