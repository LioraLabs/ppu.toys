...
fn rotation_shifts_the_surviving_set() {
    ...
    assert_eq!(
        bin.sprites.first(),
        Some(&8),
        "eval starts at the OAMADD sprite"
    );
    assert!(
        !bin.sprites.contains(&0) && !bin.sprites.contains(&7),
        "indices 0..7 dropped"
    );
    ...
    assert_eq!(
        px(&fb, 6, 100),
        backdrop,
        "index 0 dropped once the start rotates to 8"
    );
    assert_ne!(
        px(&fb, 240, 100),
        backdrop,
        "index 39 kept (opposite of rotation-off)"
    );
}
