...
#[test]
fn audio_timers_can_save_and_audio_reset_preserves_sram() {
    let mut e = LuaEngine::new();
    e.set_sram(r#"{"best":3}"#);
    e.set_source("timer(0, 255, function() sram.best = sram.best + 1 end)\nfunction frame() brightness = sram.best end")
        .unwrap();
    e.frame(0.0, 0).unwrap();
    e.frame(1.0 / 60.0, 1).unwrap();
    let saved = e.take_sram().unwrap();
    assert_eq!(saved, r#"{"best":4}"#);
    assert!(!e.audio().is_empty());
    e.reset().unwrap();
    assert!(e.audio().is_empty());
    assert_eq!(e.frame(0.0, 0).unwrap().rows[0].brightness, 4);
    assert_eq!(e.take_sram(), None);
}
