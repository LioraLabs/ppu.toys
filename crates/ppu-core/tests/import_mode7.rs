...
use ppu_core::{
    import_mode7, rgb15, unpack_rgb15, M7_MAP_DIM, M7_MAX_TILES, M7_TILE_BYTES, M7_VRAM_WORDS,
};
...
fn golden_known_image_exact_vram_and_cgram() {
    let out = import_mode7(&quadrant_rgba(), 16, 16);

    // CGRAM: palette sorted by [u8;3] order -> blue, green, red at 1..=3.
    assert_eq!(out.cgram[0], 0); // reserved transparent
    assert_eq!(out.cgram[1], rgb15(0, 0, 255)); // 0x7c00
    assert_eq!(out.cgram[2], rgb15(0, 255, 0)); // 0x03e0
    assert_eq!(out.cgram[3], rgb15(255, 0, 0)); // 0x001f
    assert_eq!(&out.cgram[4..], &[0u16; 252][..]);
    ...
    assert_eq!(out.vram[0], 0x0300); // char: tile0 px0 = 3; map (0,0) = tile 0
    assert_eq!(out.vram[1], 0x0300); // map (1,0) also tile 0 (dedup)
    assert_eq!(out.vram[63], 0x0300); // tile0 px63; map (63,0) empty = 0
    assert_eq!(out.vram[64], 0x0200); // tile1 (green) px0; map (64,0) empty
    assert_eq!(out.vram[128], 0x0101); // tile2 (blue) px0; map (0,1) = tile 1
    assert_eq!(out.vram[129], 0x0102); // map (1,1) = tile 2
    assert_eq!(out.vram[130], 0x0100); // map (2,1) beyond image = 0
    assert_eq!(out.vram[192], 0x0000); // past all char data + map row 1

    // Report.
    assert_eq!(out.report.colors, 3);
    assert_eq!(out.report.unique_tiles, 3);
    assert_eq!(out.report.tile_capacity, 256);
    assert_eq!(out.report.overflow_tiles, 0);
    assert_eq!((out.report.map_tiles_w, out.report.map_tiles_h), (2, 2));
}
...
fn golden_overflow_reports_honestly() {
    ...
    let out = import_mode7(&rgba, w, h);
    assert_eq!(out.report.unique_tiles, 512);
    assert_eq!(out.report.overflow_tiles, 256);
    assert_eq!(out.report.tile_capacity, 256);
    assert_eq!((out.report.map_tiles_w, out.report.map_tiles_h), (16, 32));
    ...
    for ty in 0..32 {
        for tx in 0..16 {
            let tile_no = (out.vram[ty * M7_MAP_DIM + tx] & 0xff) as usize;
...
fn packing_round_trip_reconstructs_quantized_image() {
    ...
    let out = import_mode7(&rgba, 16, 16);
    assert_eq!(out.vram.len(), M7_VRAM_WORDS);
    for y in 0..16 {
        for x in 0..16 {
            let (tx, ty) = (x / 8, y / 8);
            let tile_no = (out.vram[ty * M7_MAP_DIM + tx] & 0xff) as usize;
            let char_off = tile_no * M7_TILE_BYTES + (y % 8) * 8 + (x % 8);
            let idx = (out.vram[char_off] >> 8) as usize;
            assert!(idx > 0, "opaque pixel must not be transparent index 0");
            let got = unpack_rgb15(out.cgram[idx]);
