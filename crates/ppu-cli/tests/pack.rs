...
/// checked, or rendered a `controls.lua` that carries an actual poke —
...
fn pack_check_render_carries_a_real_controls_lua_poke() {
    ...
    let controls = "-- controls.lua \u{b7} generated by the Studio panels. Edit values here or in the panels.\n\
...
function apply_pokes()\n\
  ...
  hdma(scanlines.band.first, scanlines.band.last, function(y)\n\
    ...
    std::fs::write(project.join("controls.lua"), controls).unwrap();
    std::fs::write(
