...
/// `ppu init` scaffolds `controls.lua` (not the retired `timeline.lua`), and
...
fn new_project_scaffolds_controls_lua_and_it_round_trips_through_check_and_render() {
    ...
    assert_eq!(
        manifest["files"],
        serde_json::json!(["controls.lua", "main.lua"])
    );

    let controls = fs::read_to_string(Path::new(project).join("controls.lua")).unwrap();
    assert_eq!(
        controls,
        "-- controls.lua \u{b7} generated by the Studio panels. Edit values here or in the panels.\n\
...
/// explicit `apply_pokes()` call left over from the pre-`controls.lua` era.
/// That shim is gone: a toy with no `controls.lua` and an explicit
