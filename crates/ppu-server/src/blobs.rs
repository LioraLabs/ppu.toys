...
pub async fn store(state: &AppState, key: BlobKey<'_>, bytes: &[u8]) -> AppResult<()> {
    match state.cfg.blob_mode {
        BlobMode::Db => match key {
            BlobKey::Clip(id) => {
                sqlx::query("UPDATE toys SET clip=? WHERE id=?")
                    .bind(bytes)
                    .bind(id)
                    .execute(&state.pool)
                    .await?;
            ...
            BlobKey::Thumb(id) => {
                sqlx::query("UPDATE toys SET thumb=? WHERE id=?")
                    .bind(bytes)
                    .bind(id)
                    .execute(&state.pool)
                    .await?;
            ...
            BlobKey::Source(t, n) => {
                sqlx::query("UPDATE toy_sources SET payload=? WHERE toy_id=? AND name=?")
                    .bind(bytes)
                    .bind(t)
                    .bind(n)
                    .execute(&state.pool)
    ...
    match state.cfg.blob_mode {
        BlobMode::Db => {
            let row: Option<(Option<Vec<u8>>,)> = match key {
                BlobKey::Clip(id) => {
                    sqlx::query_as("SELECT clip FROM toys WHERE id=?")
                        .bind(id)
                        .fetch_optional(&state.pool)
                        .await?
                }
                BlobKey::Thumb(id) => {
                    sqlx::query_as("SELECT thumb FROM toys WHERE id=?")
                        .bind(id)
                        .fetch_optional(&state.pool)
                        .await?
                }
                BlobKey::Source(t, n) => {
                    sqlx::query_as("SELECT payload FROM toy_sources WHERE toy_id=? AND name=?")
                        .bind(t)
                        .bind(n)
                        .fetch_optional(&state.pool)
                        .await?
...
async fn clip(State(s): State<AppState>, Path(id): Path<String>) -> AppResult<Response> {
    serve_blob(s, BlobKey::Clip(&id), "video/webm").await
}
async fn thumb(State(s): State<AppState>, Path(id): Path<String>) -> AppResult<Response> {
    serve_blob(s, BlobKey::Thumb(&id), "image/png").await
}

pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/blobs/clip/{id}", get(clip))
        .route("/blobs/thumb/{id}", get(thumb))
}
