...
async fn t_route_injects_og_tags() {
    ...
    std::fs::write(
        ...
        "<head><!--OG--></head><body>app</body>",
    )
    .unwrap();
    let app = common::test_app_web(dir.path()).await;
    sqlx::query("INSERT INTO users(id,handle,avatar_hash,created_at) VALUES('1','ann','av',1)")
...
async fn t_route_escapes_html_in_title() {
    ...
    assert!(
        ...
        "title must be HTML-escaped in the meta tag"
    );
