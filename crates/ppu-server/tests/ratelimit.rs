...
#[tokio::test]
async fn api_token_updates_are_never_throttled() {
    let app = common::test_app().await;
    let sid = common::seed_session(&app.state, "1", "ann", false).await;
    let res = app
        .router
    ...
    let (id, revision) = create_toy(&app, ("authorization", &auth), false).await;
    assert_eq!(revision, 1);
    let status = put_toy(&app, ("authorization", &auth), false, &id, 1).await;
    assert_eq!(status, StatusCode::OK);
    let status = put_toy(&app, ("authorization", &auth), false, &id, 2).await;
    assert_eq!(
        status,
        StatusCode::OK,
        "API-token updates are never throttled"
    );
}
