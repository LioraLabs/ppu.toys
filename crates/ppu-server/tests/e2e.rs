...
async fn boot_seed_save_publish_wall() {
    ...
    let auth = |r: reqwest::RequestBuilder| {
        r.header("cookie", format!("ppu_sess={sid}"))
            .header("x-ppu-csrf", "1")
    };
    ...
    let form = reqwest::multipart::Form::new()
        ...
        .part(
            "clip",
            reqwest::multipart::Part::bytes(b"clip".to_vec())
                .file_name("c.webm")
                .mime_str("video/webm")
                .unwrap(),
        )
        .part(
            "thumb",
            reqwest::multipart::Part::bytes(b"thmb".to_vec())
                .file_name("t.png")
                .mime_str("image/png")
                .unwrap(),
        );
    let pub_res = auth(http.post(format!("{base}/api/toys/{id}/publish")))
        .multipart(form)
        .send()
        .await
        .unwrap();
    ...
    let wall: serde_json::Value = http
        .get(format!("{base}/api/toys?sort=recent"))
        .send()
        .await
        .unwrap()
        .json()
