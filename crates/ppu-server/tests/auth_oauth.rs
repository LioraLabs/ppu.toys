...
async fn mock_discord() -> String {
    let app = Router::new()
        ...
        .route(
            "/users/@me",
            get(|| async {
                Json(serde_json::json!({ "id": "9001", "username": "trinity", "avatar": "abc" }))
            }),
        );
...
async fn callback_upserts_user_mints_session_sets_cookie() {
    ...
    let res = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/auth/callback?code=xyz&state=st")
                .header("cookie", "ppu_oauth_state=st")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    let (h,): (String,) = sqlx::query_as("SELECT handle FROM users WHERE id='9001'")
        .fetch_one(&app.state.pool)
        .await
        .unwrap();
    assert_eq!(h, "trinity");
...
async fn callback_rejects_state_mismatch() {
    ...
    let res = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/auth/callback?code=xyz&state=st")
                .header("cookie", "ppu_oauth_state=DIFFERENT")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn callback_rejects_banned_discord_id() {
    ...
    sqlx::query("INSERT INTO bans(discord_id,created_at) VALUES('9001',1)")
