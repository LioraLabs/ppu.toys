...
async fn fork_copies_files_and_sources_for_caller() {
    ...
    sqlx::query("INSERT INTO toy_sources(toy_id,name,kind,payload) VALUES('orig','bg1','bg',?)")
        .bind(&b"data"[..])
    ...
    let res = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/toys/orig/fork")
                .header("cookie", format!("ppu_sess={forker}"))
                .header("x-ppu-csrf", "1")
                .body(Body::empty())
                .unwrap(),
        )
    ...
    let b = axum::body::to_bytes(res.into_body(), 1 << 20)
        .await
        .unwrap();
    let nid = serde_json::from_slice::<serde_json::Value>(&b).unwrap()["id"]
        .as_str()
        .unwrap()
        .to_string();

    let (author, forked, state): (String, Option<String>, String) =
        sqlx::query_as("SELECT author_id,forked_from,state FROM toys WHERE id=?")
            .bind(&nid)
    ...
    let (n, payload): (i64, Option<Vec<u8>>) =
        sqlx::query_as("SELECT COUNT(*), MAX(payload) FROM toy_sources WHERE toy_id=?")
            .bind(&nid)
            .fetch_one(&app.state.pool)
            .await
            .unwrap();
