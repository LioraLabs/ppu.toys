...
describe("serializeWorkspace", () => {
  it("demo context: every bundled asset becomes a payload-bearing source", () => {
    const state: OpenSketchState = {
      context: { kind: "demo", demoId: "dusk-parallax" },
      dirty: false,
      session: 0,
    };
    ...
    expect(result.sources.every((s) => typeof s.payload === "string" && s.payload.length > 0)).toBe(
      true,
    );
  ...
  it("forked-demo sketch: demo assets replay AND user-added sources are included", () => {
    const sketch = newSketchObject(
      "x",
      dusk.files!,
      [userSource("custom", new Uint8Array([9]))],
      "dusk-parallax",
    );
    const state: OpenSketchState = {
      context: { kind: "sketch", sketch },
      dirty: false,
      session: 0,
    };
    const result = serializeWorkspace(state, fakeConvert);
  ...
  it("user source overrides a same-named demo asset", () => {
    const sketch = newSketchObject(
      "x",
      dusk.files!,
      [userSource("sky", new Uint8Array([42]))],
      "dusk-parallax",
    );
    const state: OpenSketchState = {
      context: { kind: "sketch", sketch },
