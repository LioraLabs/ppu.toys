...
export function SourcePreview({
  ...
  const model = useMemo(
    () => buildPreviewModel(kind, meta, payload, cellSize),
    [kind, meta, payload, cellSize],
  );
    ...
    <div className="srcpv">
      <div className="srcpv-tools" role="group" aria-label="Preview view">
        <button
          ...
          aria-pressed={view === "snes"}
          onClick={() => setView("snes")}
        >
          snes
        </button>
        <button
        ...
        {canTint && (
          <button
            type="button"
            className="srcpv-tool srcpv-tool-right"
            aria-pressed={tint}
            onClick={() => setTint((t) => !t)}
          >
            palettes
        ...
        <div className="srcpv-swatches" aria-label="Resulting palettes">
          {model.palettes.map((p, pi) => (
            <div className="srcpv-swatchrow" key={pi}>
              <span
                className="srcpv-swatchrow-label"
                style={tint ? { background: palTint(pi) } : undefined}
              >
                p{pi}
              </span>
              {p.map((c, ci) => (
                <span
                  className="srcpv-swatch"
                  key={ci}
                  style={{ background: css555(c) }}
                  title={`$${c.toString(16).padStart(4, "0")}`}
                />
              ))}
            </div>
          ))}
      ...
      <div className="srcpv-budget">
        {model.budget.map((b) => (
          <span key={b} className="srcpv-chip">
            {b}
          </span>
        ))}
      </div>
      ...
      {kind === "bg" && (
        <div className="srcpv-note">Depth/mode mismatches are reported at bind, not here.</div>
      )}
    </div>
  );
}
