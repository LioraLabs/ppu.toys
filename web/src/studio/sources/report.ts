...
function overflowLine(o: ImportOverflow): string {
  switch (o.kind) {
    case "Cropped":
      return `cropped to ${o.max_px}px`;
    case "Colors":
      return `Colors: ${o.unique} unique > ${o.budget} budget`;
    case "Palettes":
      return `Palettes: needs ${o.needed}, ${o.remapped_tiles} tiles remapped`;
    case "Tiles":
      return `Tiles: ${o.unique} unique, kept ${o.kept}`;
    case "TileSize16":
      return "16px tiles not supported";
  }
}
...
export function sourceReportView(report: SourceReport): { budget: string[]; warns: string[] } {
  if (report.mode === "m7") {
    ...
    return {
      budget: [
        `${r.unique_tiles}/${r.tile_capacity} tiles`,
        `${r.colors}/256 colors`,
        `${r.map_tiles_w}×${r.map_tiles_h} map`,
      ],
