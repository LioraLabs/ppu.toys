...
/** A representative Lua sketch source for the CodeEditor story — exercises
 *  comments, keywords, numbers, strings and a couple of PPU builtin calls so
 *  syntax highlighting renders something non-trivial with no wasm core. */
export const luaSource = `-- main.lua — bouncing sprite demo
local x, y = 24, 40
local dx, dy = 1.5, 1.0
...
function _update()
  x = x + dx
  y = y + dy
  if x < 0 or x > 240 then dx = -dx end
  if y < 0 or y > 208 then dy = -dy end
end

function _draw()
  cls(0)
  spr(0, x, y)
  print("ppu.toys", 8, 8, 7)
end
`;

/** A multi-file sketch in execution order for the FileTabs story: the
 *  generated pokes.lua (index 0, pinned) plus three editable source files.
 *  `main.lua` carries the shared `luaSource` so the FileTabs and CodeEditor
 *  fixtures agree on the "active" file's contents. */
export const sketchFiles: SourceFile[] = [
  { name: "pokes.lua", source: "-- generated by the poke inspector\n" },
  { name: "main.lua", source: luaSource },
  { name: "player.lua", source: "-- player.lua\nlocal Player = {}\nreturn Player\n" },
  { name: "enemies.lua", source: "-- enemies.lua (has a syntax error)\nlocal e =\n" },
];
